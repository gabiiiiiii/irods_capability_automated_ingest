...
class scanner_base(object):
    ...
    def syncronize_chunk(self, meta, chunk):
        ...
        task_queue(file_q_name, sync_files_meta).add(irods_capability_automated_ingest.irods_task.sync_files)
...
class filesystem_scanner(scanner_base):
    ...
    def get_iter(self, meta):
        ...
        task_queue(file_q_name, meta).add(irods_capability_automated_ingest.irods_task.sync_dir)
    ...
    def recursively_sync_obj(self, meta, obj):
        ...
        if obj.is_dir() and not obj.is_symlink() and not obj.is_file():
            ...
            task_queue(path_q_name, sync_dir_meta).add(irods_capability_automated_ingest.irods_task.sync_path)
            return None
...
def scanner_factory(meta, scanner_type):
    return eval(scanner_type)(meta)
