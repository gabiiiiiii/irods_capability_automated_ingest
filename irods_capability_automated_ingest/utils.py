...
def enqueue_task(task, meta):
    ...
    if job.is_stopping():
        ...
        logger.info('async_job_name_stopping', task=meta['task'], path=meta['path'], job_name=job.name())
        pass

    logger.info('incr_job_name', task=meta['task'], path=meta['path'], job_name=job.name())
    job.tasks_handle().incr()
    task_id = str(uuid1())
    timeout = custom_event_handler(meta).timeout()
    job.count_handle().rpush(task_id)
    task.s(meta).apply_async(queue=meta['queue_name'], task_id=task_id, soft_time_limit=timeout)
