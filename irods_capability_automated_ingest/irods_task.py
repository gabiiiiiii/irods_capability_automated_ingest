...
def sync_path(self, meta, scanner_type='filesystem_scanner'):
    syncer = scanner_factory(meta, scanner_type)
    syncer.sync_path(self, meta)

@app.task(bind=True, base=IrodsTask)
def sync_dir(self, meta, scanner_type='filesystem_scanner'):
    syncer = scanner_factory(meta, scanner_type)
    syncer.sync_entry(self, meta, "dir", sync_irods.sync_data_from_dir, sync_irods.sync_metadata_from_dir)

@app.task(bind=True, base=IrodsTask)
def sync_files(self, in_meta, scanner_type='filesystem_scanner'):
    chunk = in_meta["chunk"]
    meta = in_meta.copy()
    syncer = scanner_factory(meta, scanner_type)
